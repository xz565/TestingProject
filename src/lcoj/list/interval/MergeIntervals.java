...
public class MergeIntervals {
    ...
    for (Interval newInterval : intervals) {
      ...
      for (Interval existingInterval : result) {
        ...
        if (isOverlap(newInterval, existingInterval)) {
          existingInterval.start = (existingInterval.start < newInterval.start ? existingInterval.start : newInterval.start);
          existingInterval.end = (existingInterval.end > newInterval.end ? existingInterval.end : newInterval.end);
      ...
      if (isNew) {
        Interval inteval = new Interval(newInterval.start, newInterval.end);
        result.add(inteval);
      }
    }
  ...
  private boolean isOverlap(Interval newInternval, Interval exisInterval) {
    ...
    return isOverlapHelper(newInternval, exisInterval) || isOverlapHelper(exisInterval, newInternval);
  }


  private boolean isOverlapHelper(Interval i1, Interval i2) {

    return (i1.start >= i2.start && i1.start <= i2.end) || (i1.end >= i2.start && i1.end <= i2.end);
  }
